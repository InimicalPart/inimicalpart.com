...
import { isAdminCheck, approveAppeal } from "@/utils/conn"
...
export async function POST(req: NextRequest, {params}: {params: {USER_ID: string, OFFENSE_ID: string, SERVER_ID: string}}) {
    ...
    const response = await approveAppeal(botConnection.connection, userID, params.OFFENSE_ID).catch(e=>e) as any
