...
import { isAdminCheck, getUsersOffenses } from "@/utils/conn"
...
export async function GET(req: NextRequest, {params}: {params: {USER_ID: string, SERVER_ID: string}}) {      
    ...
    const offenses = await getUsersOffenses(botConnection.connection, params.USER_ID).catch(e=>e) as any

    if (offenses == "TIMEOUT") return NextResponse.json({ error: "QUERY_TIMEOUT", message: "Query timed out" }, { status: 500 })
    if (offenses?.result.error == "INVALID_ACTION") return NextResponse.json({ error: "INVALID_ACTION", message: "This action is invalid for this offense." }, { status: 400 })

    return NextResponse.json({ offenses: offenses.result.offenses }, { status: 200 })
