...
import { isAdminCheck, getAppeal } from "@/utils/conn"
...
export async function GET(_req: NextRequest, {params}: {params: {USER_ID: string, OFFENSE_ID: string, SERVER_ID: string}}) {
    ...
    const offense = await getAppeal(botConnection.connection, params.USER_ID, params.OFFENSE_ID).catch(e=>e) as any
    ...
    return NextResponse.json({ id: userID, appeal: offense.result }, { status: 200 })
}