...
export default function Etc({ session: sessionUser, userID, SERVER_ID }: { session: any, userID: string, SERVER_ID: string }) {
    ...
    function toggleAppeal(offense: Offense) {
      ...
      setToggling(true)
      fetch(apiLoc + "/admin/users/"+userID+"/offenses/"+offense.id+"/toggle-appealment", {method: "POST"})
      .then(res=>res.json())
      .then(data=>{
        if (data.error == "UNAVAILABLE") {
          redirect(appealURLPrefix + "/")
