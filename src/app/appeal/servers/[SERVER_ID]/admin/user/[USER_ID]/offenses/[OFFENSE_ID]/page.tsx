...
export default async function Page({
    ...
    return <Etc SERVER_ID={params.SERVER_ID} session={{username: user?.username as string, imageUrl: user?.imageUrl as string, firstName: user?.firstName as string, discord: {id: user?.externalAccounts[0].externalId, username: user?.externalAccounts[0].username} }} OFFENSE_ID={params.OFFENSE_ID} USER_ID={params.USER_ID} />
