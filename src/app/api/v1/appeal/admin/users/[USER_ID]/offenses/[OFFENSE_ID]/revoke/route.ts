...
import { getOffenses, getUser, getUserInfo, getUsers, revokeOffense, saveEmail, toggleAppealment } from "@/utils/iris"
...
export async function POST(req: NextRequest, {params}: {params: {USER_ID: string, OFFENSE_ID: string}}) {
    ...
    const response = await revokeOffense(userID, params.OFFENSE_ID).catch(e=>e) as any
